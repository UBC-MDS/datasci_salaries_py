...
def plot(xmax):
    ...
    chart_map = alt.Chart(datamap).mark_geoshape().encode( 
    ...
    rect = rect.transform_filter(map_click)
    bar = bar.transform_filter(map_click)
    fchart = alt.vconcat(rect, bar, spacing=0)
    
    ### Box
    box = plot_box()
    box = box.transform_filter(map_click)
